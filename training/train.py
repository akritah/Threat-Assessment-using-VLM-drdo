...
from training.dataset import create_sample_dataset, load_training_datasets

_PROJECT_ROOT = Path(__file__).resolve().parent.parent


def _build_bnb_config(config: TrainingConfig) -> BitsAndBytesConfig:
...
def train(config: TrainingConfig, resume: bool = False) -> Path:
    ...
    logging.info("Loading base model: %s", config.base_model_id)
    ...
    logging.info("Adapter saved to: %s", output_path)
